...
## 2. Point a TrainConfig at the converted data
...
0% success (both a 0/25 randomized-layout eval and a 0/15 fixed-scene eval across the 3 recorded
scenes) -- from an earlier version of this pipeline that converted to a Groot-LeRobot format
instead of the plain format above (`LeRobotRobocasaDataConfig`/Groot modality.json, since replaced;
checkpoints and eval videos from that run are still under
`checkpoints/pi0_robocasa_coffeesetupmug_hitl_lora*/` for reference). Verified numerically
equivalent to what this pipeline produces (same final normalized training values), so the result
still holds: not evidence that 1-3 demos is too little data, but that these "semantic_corrections"
HITL demos never reach the task-success reward in the source data either (see caveat below) --
straight behavior cloning on them reproduces the documented failure mode, not a policy that
completes the task.

## Caveats

- These "semantic_corrections" HITL hdf5s record a *monitored* rollout annotated with what went
  wrong (collisions, missed placements, etc.) -- `rewards`/`dones` are 0/False throughout in the
  files used so far, i.e. they may not be successful demonstrations. Plain behavior cloning on
  them reproduces the documented failure mode. How `correction_semantic_annotations` should
  factor into training is unresolved -- confirm with whoever owns that pipeline before drawing
  conclusions from eval results.

...
Copied verbatim from Arpitrf/semantic_corrections (semantic_corrections/hitl/mug_weld.py,
`point-set` branch @ c5c6cbe), except the import below (flat layout, matching sim_state.py/
hitl_obs.py/hitl_env.py already copied into this same directory). Only change: this project's
`configs/tasks/coffee.yaml` sets `weld_on_grasp: true` for CoffeeSetupMug specifically -- our
own eval_fixed_scene.py didn't apply this at all, so evaluated rollouts let the mug slip/rotate
freely after grasp instead of rigidly following the gripper the way the training demos (recorded
through run_pi0_hitl.py, which does apply it) actually behaved. See eval_fixed_scene.py for the
wiring that replicates run_pi0_hitl.py's `_maybe_apply_grasp_weld`/`_maybe_break_weld_on_gripper_open`.
"""
